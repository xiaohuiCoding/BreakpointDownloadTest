...
- (void)deleteFile:(NSString *)url
{
    ...
    if ([fileManager fileExistsAtPath:HSFileFullpath(url)]) {
        ...
        NSUInteger taskId = [self getTask:url].taskIdentifier;
        
        // 删除沙盒中的资源
        [fileManager removeItemAtPath:HSFileFullpath(url) error:nil];
        // 取消任务
        [[self getTask:url] cancel];
        // 删除任务
        [self.tasks removeObjectForKey:HSFileName(url)];
        // 关闭流（此时task已经被删除了，无法通过task找到对应的model，只能通过保存的taskId来找到）
        [[self getSessionModel:taskId].stream close];
        // 删除model
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSHTTPURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler
{
    ...
    NSInteger length1 = [response.allHeaderFields[@"Content-Length"] integerValue];
    NSInteger length2 = HSDownloadLength(sessionModel.url);
